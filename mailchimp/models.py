...
class QueueManager(models.Manager):
    def queue(self, campaign_type, contents, list_id, template_id, subject,  # NOQA
            ...
            tracking_opens=True, tracking_html_clicks=True,
            tracking_text_clicks=False, title=None, authenticate=False,
            google_analytics=None, auto_footer=False, auto_tweet=False,
            segment_options=False, segment_options_all=True,
        ...
        """
        kwargs = locals().copy()
        kwargs['segment_options_conditions'] = simplejson.dumps(
...
class Queue(models.Model):
    ...
    def send(self):
        ...
        content_data = dict([(str(k), v) for k, v in simplejson.loads(
            self.contents).items()])
        ...
        camp = c.create_campaign(self.campaign_type, c.get_list_by_id(
            self.list_id),
    ...
    def object(self):
        """
        The object might have vanished until now, so triple check that it's
        there!
        """
        if self.object_id:
    ...
    def get_object_admin_url(self):
        ...
        name = 'admin:%s_%s_change' % (
            self.object._meta.app_label, self.object._meta.module_name)
    ...
    def can_dequeue(self, user):
        ...
        return user.has_perm('mailchimp.can_send') and user.has_perm(
            'mailchimp.can_dequeue')


class CampaignManager(models.Manager):
    def create(self, campaign_id, segment_opts, content_type=None,
               object_id=None, extra_info=[]):
        con = get_connection()
...
class Campaign(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('mailchimp_campaign_info',
    ...
    def mc(self):
        try:
            if not hasattr(self, '_mc'):
                self._mc = get_connection().get_campaign_by_id(
                    self.campaign_id)
            return self._mc
        except:
